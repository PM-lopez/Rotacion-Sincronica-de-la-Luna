...
def rk4_method_second_order(f, t0, y0, v0, tf, h):
    """ Metodo de Rk4 de segundo orden en 1D. """
    t = np.arange(start=t0, step=h, stop=tf + h, dtype=float)
    ...
    for n in range(len(t) - 1):
        ...
        k2 = h * f(t[n] + h/2, y[n] + m1/2, v[n] + k1/2)
        m2 = h * (v[n] + k1/2)

        k3 = h * f(t[n] + h/2, y[n] + m2/2, v[n] + k2/2)
        m3 = h * (v[n] + k2/2)

        k4 = h * f(t[n] + h, y[n] + m3, v[n] + k3)
        m4 = h * (v[n] + k3)
...
def euler_method_second_order_2D(f, t0: float, r0: tuple, v0: tuple, tf: float, h: float = 0.0001) -> tuple:
...
def euler_method_second_order(f, t0, y0, v0, tf, h):
    """ Metodo de Euler de segundo orden en 1D. """
    t = np.arange(start=t0, step=h, stop=tf + h, dtype=float)
    y = np.zeros_like(t, dtype=float)
    v = np.zeros_like(t, dtype=float)
